import formularProblemaArtificial, { ProblemaArtificial } from "./adicionarArtificiais";
...
function solucaoBasicaInicial(problemaArtificial: ProblemaArtificial) {
	...
	const invB = inversa(B, problemaArtificial.b);
...
function escolherVariavelQueEntra(custosRelativos: number[], variaveisNaoBasicas: number[]) {
...
function calcularDirecaoSimplex(
	...
	const y = multiplicarMatriz(invB, aNkColuna);
...
export default function fase1(problema: Problema) {
	const problemaArtificial = formularProblemaArtificial(problema.mat, problema.ind);
